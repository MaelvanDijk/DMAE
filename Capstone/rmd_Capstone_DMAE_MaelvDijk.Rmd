...
### Ontwikkeling van de vraag naar Skills
...
df_indeed_skills %>%
    dplyr::filter(skills %in% names(top_skills)) %>% # filter df based on most sought skill
    ggplot(
        aes(x = reorder(
            skills, skills, function(x) + length(x)), # decreasing order skill
            fill= job_type
            )) +
    geom_bar() +
