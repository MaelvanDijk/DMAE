...
plot_indeed_timeseries_data(df_indeed_skills)
...
plot_indeed_timeseries_data(df_indeed_skills_date_filtered)
...
```{r}
# get top 5 skills
# important_skills <- table(df_indeed_skills$skills) %>% sort() %>% tail(5)
...
# top_companies <- table(df_indeed_skills$Company) %>% sort() %>% tail(20)
...
```{r}
df_indeed_final %>%
  filter(listing_date >= "2022-02-05") %>%
  group_by(listing_date, job_type) %>%
  ...
  geom_line() +
  facet_wrap(vars(job_type), ncol=1)+
...
t <- df_indeed_final %>%
  filter(Company%in% companies_of_interest) %>%
  group_by(listing_date, Company, .groups="keep") %>%
  summarise(req_per_day= n()) %>%
  group_by(
    week= paste(as.character(year(listing_date)),
          as.character(week(listing_date)),
          sep=""),
    Company) %>%
  summarise(req_per_day= sum(req_per_day), .groups="keep") 
