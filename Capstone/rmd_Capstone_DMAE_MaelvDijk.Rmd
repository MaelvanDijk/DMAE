...
### Ontwikkeling vraag naar Skills  {.tabset}
...
buiten het feit dat HoB haar activiteiten hier niet op moet focussen is
levert het ons geen extra informatie op.
...
Een volgende stap is trend analyse. Omdat de data niet vergenoeg terug
...
van opleidingen en het opleiden van kandidaten. Wel kan getracht worden
op basis van onderstaande grafiek (en bijbehorende code) een algemeen
beeld te vormen van de markt en haar beweging.

```{r}
top_5_skills <- top_skills <- table(df_indeed_skills$skills) %>% sort() %>% tail(5)
