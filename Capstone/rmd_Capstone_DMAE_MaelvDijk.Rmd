...
Tijdens een recente informatieavond van HoB zijn doelstellingen voor
2022 gepresenteerd waar ook de ontwikkeling van nieuwe opdrachtgevers en
nieuwe data professionals (30 stuks) naar voren komt. De combinatie van
de uitstroom en de doelstellingen vormen de aanleiding voor dit
onderzoek.

Het is hier ook relevant om te melden dat HoB veel van haar IT processen
en functionaliteiten in koopt. Dit betekent dat er weinig intern wordt
gebouwd. Zodoende zijn er nog geen data mining activiteiten.
...
D001 is na vooronderzoek gekozen als basis dataset (zie ook hoofdstuk
"Addendum" voor aansluitende analyses). Hierin staan onder andere
(hard)skills, functie type (data analist, -engineer, -scientist). Dit
legt een goede basis voor dit onderzoek, de functie type sluiten
namelijk aan op de specialisaties die HoB biedt. Andere overwogen
datasets miste vaak de kolom met vaardigheden of \<200 observaties
aangaande data functies. Vandaar dat de keuze op D001 is komen te liggen
...
API's en afwezigheid van relevante datasets. Indeed is gekozen omdat dit
een van de grotere vacaturebanken is. Ook kan de scraper (na bewerking)
andere functies binnen halen niet gerelateerd aan het data domein. Op
deze manier is de scraper en de vacaturebank multipurpose.
...
het overzicht hierboven toont dat het aantal unieke "Skills" bijna
gelijk is aan het aantal unieke rijen. Dit betekent echter niet dat er
zoveel verschil zit tussen de vacatures. Het is namelijk zo dat [SQL,
Python] en [Python, SQL] hetzelfde betekenen maar wordt gezien als twee
unieke rijen. Deze informatie wordt gebruikt in latere ETL stappen.
Daarnaast is opvallend dat de verdeling tussen data analist (31%), data
engineer (24%) en data scientist (45%) niet volledig evenredig is.

Dataset D002 bevat Indeed data. Hiervoor is er gezocht op functies met
het woord "data". D002 bestaat in de uit meerdere losse bestanden (een
voor elke dag dat er een run is geweest van de scraper). als voorbeeld
wordt hier gekeken naar: ".\\Data_raw\\D002\\2022-03-08 scraped indeed
data.csv". Hieronder weer de voornaamste kenmerken.
...
2.  Er zijn geen volledige dubbele rijen of rijen die volledig leeg
    zijn.
3.  Wel bevatten 283 rijen unieke URL's en 266 rijen unieke
    beschrijvingen dit zou toch op dubbele vacatures kunnen duiden.
...
het de indruk dat het salaris geboden door HoB doorgaans (2, 3, 8, en
NA) beter is dan het salaris van Indeed (jaren 6 en 7 buiten beschouwing
gelaten).
...
df_indeed_skills_date_filtered %>%
  filter(
    ...
    skills %in% important_skills_full_pop) %>%
  ggplot(
    aes(
        x = reorder(
          skills,
